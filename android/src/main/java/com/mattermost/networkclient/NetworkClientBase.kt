...
internal open class NetworkClientBase(
    ...
    var shouldCollectMetrics: Boolean = false
    var metricsEventFactory: MetricsEventFactory? = null
    ...
    internal fun initCollectMetrics(options: ReadableMap?) {
    ...
    internal fun emitInvalidPinnedCertificateError() {
        ...
        data.putString(
            "errorDescription",
            "Server trust evaluation failed due to reason: Certificate pinning failed for host ${
                URI(baseUrlString).host
            }"
        )
        ApiClientModuleImpl.sendJSEvent(ApiClientEvents.CLIENT_ERROR.event, data)
    ...
    internal fun buildHandshakeCertificates(): HandshakeCertificates? {
        if (baseUrl == null)
            return null
    ...
    internal fun setClientTimeoutInterceptor(options: ReadableMap?) {
        ...
        var connectTimeout = TimeoutInterceptor.DEFAULT_CONNECT_TIMEOUT
    ...
    private fun createRetryInterceptor(
        ...
        if (retryConfig.hasKey("statusCodes")) {
            val codes = retryConfig.getArray("statusCodes")
                ?.let { it.toArrayList().map { code -> (code as Number).toInt() } }?.toSet()
            if (codes != null) {
        ...
        if (retryType == RetryTypes.LINEAR_RETRY) {
            ...
            var exponentialBackoffBase =
                ExponentialRetryInterceptor.DEFAULT_EXPONENTIAL_BACKOFF_BASE
            if (retryConfig.hasKey("exponentialBackoffBase")) {
                exponentialBackoffBase = retryConfig.getDouble("exponentialBackoffBase")
            }
            var exponentialBackoffScale =
                ExponentialRetryInterceptor.DEFAULT_EXPONENTIAL_BACKOFF_SCALE
    ...
    internal fun getCertificatesFingerPrints(): Map<String, List<String>> {
        val fingerprintsMap = mutableMapOf<String, MutableList<String>>()
        val assetsManager = context.assets
        val certFiles = assetsManager.list(CERTIFICATES_PATH)
            ?.filter { it.endsWith(".cer") || it.endsWith(".crt") } ?: return emptyMap()
